...
def _runSSE(method='', symbols=None, on_data=None, token='', version=''):
    ...
    if symbols:
        ...
        return _streamSSE(_SSE_URL_PREFIX.format(channel=method, symbols=symbols, token=token, version=version),
                          on_data=on_data)
    return _streamSSE(_SSE_URL_PREFIX_ALL.format(channel=method, symbols=symbols, token=token, version=version),
                      on_data=on_data)


def topsSSE(symbols=None, on_data=None, token='', version=''):
    """
    TOPS provides IEX’s aggregated best quoted bid and offer position in near real time
    for all securities on IEX’s displayed limit order book.
...
def lastSSE(symbols=None, on_data=None, token='', version=''):
    """
    Last provides trade data for executions on IEX. It is a near real time, intraday API
    that provides IEX last sale price, size and time.
...
def deepSSE(symbols=None, channels=None, on_data=None, token='', version=''):
    ...
    return _streamSSE(_SSE_DEEP_URL_PREFIX.format(symbols=symbols, channels=channels, token=token, version=version),
                      on_data)
...
def tradesSSE(symbols=None, on_data=None, token='', version=''):
    ...
    """
    symbols = _strCommaSeparatedString(symbols)
    return _streamSSE(_SSE_DEEP_URL_PREFIX.format(symbols=symbols, channels='trades', token=token, version=version),
                      on_data)
...
def opHaltStatusSSE(symbols=None, on_data=None, token='', version=''):
    ...
    IEX disseminates a full pre-market spin of Operational halt status messages
    indicating the operational halt status of all securities.
    ...
    If a security is absent from the dissemination, firms should assume that the security is being treated
    as operationally halted in the IEX Trading System at the start of the Pre-Market Session.
...
def ssrStatusSSE(symbols=None, on_data=None, token='', version=''):
    """
    In association with Rule 201 of Regulation SHO, the Short Sale Price Test Message
    is used to indicate when a short sale price test restriction is in effect for a security.

    IEX disseminates a full pre-market spin of Short sale price test status messages indicating
    the Rule 201 status of all securities. After the pre-market spin,
    IEX will use the Short sale price test status message in the event of an intraday status change.
...
def tradingStatusSSE(symbols=None, on_data=None, token='', version=''):
    ...
    After the pre-market spin, IEX will use the Trading status message to relay changes in trading status for
    an individual security. Messages will be sent when a security is:
